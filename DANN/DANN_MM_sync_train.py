...
from DANN.DANN_MM_sync_model import SyncDualEncoderDANN
...
def train(seed=42, epochs=30, batch_size=64, lr=1e-3):
    ...
    print(f"\n{'='*60}")
    print(f" DANN-MM Sync Training  |  seed={seed}  device={device}")
    print(f"{'='*60}")
    ...
    weight_path = f'weights/dann_mm_sync_seed{seed}_best.pth'

    model         = SyncDualEncoderDANN(num_classes=num_classes).to(device)
    ...
    save_confusion_matrix(src_labels, src_preds, class_names,
                          f'results/dann_mm_sync_seed{seed}_source_cm.png',
                          f'DANN-MM Sync Source (seed={seed}, acc={src_acc:.4f})')
    save_confusion_matrix(tgt_labels, tgt_preds, class_names,
                          f'results/dann_mm_sync_seed{seed}_target_cm.png',
                          f'DANN-MM Sync Target (seed={seed}, acc={tgt_acc:.4f})')
