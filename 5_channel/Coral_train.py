"""CORAL 학습 스크립트 (단일 백본, 5채널 동기화 입력, multi-seed 지원).

TCN baseline 백본의 feature 에 CORAL 손실(source/target covariance alignment)을
추가해 Source(Samsung1) 라벨만으로 Target(Samsung2) 운동 분류 성능을 끌어올린다.
각 배치는 Source / Target 을 분리 forward 한다. MMD 실험과 동일한 백본 아키텍처에
정렬 손실만 CORAL 로 바꾼 비교군이다.
"""
import os
import sys
import argparse
...
# ----------------------------------------------------------------------
# 백본 래퍼: DANNModel에서 (분류 logits, feature) 를 함께 반환
# ----------------------------------------------------------------------
class CORALWrapper(nn.Module):
    """DANNModel을 감싸 (분류 logits, feature) 를 반환한다.

    DANN/MMD/CDAN과 동일한 DANNModel 백본을 사용해 공정한 비교를 보장한다.
    domain_classifier는 사용하지 않는다.
    """
    ...
    def forward(self, x):
        features = self.model.feature_extractor(x).squeeze(-1)
        return self.model.label_classifier(features), features
...
def train_coral(seed=42, epochs=30, batch_size=64, lr=1e-3, lambda_coral=1.0, save_cm=False):
    ...
    save_path = os.path.join(WEIGHT_DIR, f"5ch_coral_seed{seed}_best_model.pth")

    train_loader, val_loader, tgt_train_loader, tgt_val_loader, num_classes, le = \
        get_dataloaders(batch_size=batch_size)
    class_names = le.classes_

    model = CORALWrapper(DANNModel(in_channels=5, num_classes=num_classes)).to(device)
    ...
    if save_cm:
        ...
        save_confusion_matrix(
            ...
            os.path.join(RESULT_DIR, f"5ch_coral_seed{seed}_source_cm.png"),
            f"CORAL 5ch Source (seed={seed}, Val Acc: {best_val_acc:.1f}%)", cmap="Greens")
        save_confusion_matrix(
            t_true, t_preds, class_names,
            os.path.join(RESULT_DIR, f"5ch_coral_seed{seed}_target_cm.png"),
            f"CORAL 5ch Target (seed={seed}, Src: {best_val_acc:.1f}% vs Tgt: {best_target_acc:.1f}%)",
            cmap="Greens")
...
if __name__ == "__main__":
    ...
    if args.multi_seed:
        ...
        summarize_results(results, method_name="CORAL (5-channel)",
                          save_path=os.path.join(RESULT_DIR, "5ch_coral_summary.txt"))
    else:
        train_coral(
