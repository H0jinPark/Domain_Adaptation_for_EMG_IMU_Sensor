"""DANN-MM-Stack 학습 스크립트 (2-phase: 모달별 CL 사전학습 → (B,2,256) stack 융합 DANN).

Phase 1 - DANN_MM_ssl 과 정확히 동일한 모달별 contrastive 사전학습 (코드 재사용).
          기존 CL 인코더 가중치(dann_mm_ssl_*_encoder_seed*.pth)가 있으면 재사용하고,
          없으면 새로 학습한다.
Phase 2 - 사전학습 인코더를 DualEncoderDANN_Stack 에 이식해 DANN 파인튜닝.
          융합을 옆 concat((B,512)) 대신 (B,2,256) stack + 1D conv 로 수행한다.

→ DANN_MM_ssl 과 Phase 1 이 동일하므로, 두 결과 비교 시 stack 융합의 효과만 분리된다.
"""
import os
import sys
import argparse
...
from DANN.DANN_experiment.DANN_MM_stack_model import DualEncoderDANN_Stack


# -----------------------------------------------------------------------
# Phase 2: (B,2,256) stack 융합 DANN 파인튜닝
# -----------------------------------------------------------------------
def finetune(seed, emg_path, imu_path, epochs, lr, device,
             src_loader, val_loader, tgt_loader, tgt_val_loader,
             num_classes, class_names):
    """사전학습 인코더를 로드해 DualEncoderDANN_Stack 를 DANN 방식으로 파인튜닝한다."""

    print(f"\n{'='*55}")
    print(f" Phase 2: DANN Fine-tuning (stack fusion)  |  finetune_epochs={epochs}")
    print(f"{'='*55}")

    model = DualEncoderDANN_Stack.from_pretrained(emg_path, imu_path, num_classes).to(device)
    ...
    weight_path  = f'weights/dann_mm_stack_seed{seed}_best.pth'
    best_val_acc = 0.0
    ...
    save_confusion_matrix(src_labels, src_preds, class_names,
                          f'results/dann_mm_stack_seed{seed}_source_cm.png',
                          f'DANN-MM-Stack Source (seed={seed}, acc={src_acc:.4f})')
    save_confusion_matrix(tgt_labels, tgt_preds, class_names,
                          f'results/dann_mm_stack_seed{seed}_target_cm.png',
                          f'DANN-MM-Stack Target (seed={seed}, acc={tgt_acc:.4f})')
    return src_acc, tgt_acc


# -----------------------------------------------------------------------
# 전체 학습: Phase 1 (CL, 재사용 가능) → Phase 2 (stack 융합 DANN)
# -----------------------------------------------------------------------
def train(seed=42, ssl_epochs=50, finetune_epochs=30,
    ...
    print(f"\n{'='*55}")
    print(f" DANN-MM-Stack Training  |  seed={seed}  device={device}")
    print(f"{'='*55}")
