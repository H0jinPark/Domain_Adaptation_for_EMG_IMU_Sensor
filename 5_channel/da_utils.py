"""5채널 도메인 적응 실험 공용 유틸리티.

5_channel/ 폴더의 MMD/Coral/DANN/CDAN 학습 스크립트가 공통으로 쓰는 함수 모음.
...
def evaluate(model, loader, device, needs_alpha=False):
    ...
    needs_alpha=True 면 DANN/CDAN 처럼 forward(x, alpha) 시그니처를 갖는 모델로
    보고 alpha=0.0 으로 호출한다 (평가 시 GRL 비활성화). 두 경우 모두 forward 의
    첫 번째 반환값을 클래스 logits 로 사용한다.
    """
    model.eval()
    preds, labels = [], []
    for x, y in loader:
        x = x.to(device)
        out = model(x, alpha=0.0)[0] if needs_alpha else model(x)[0]
...
def summarize_results(results, method_name="", save_path=None):
    ...
    save_path 가 주어지면 동일한 요약을 텍스트 파일로도 저장한다. multi-seed 실행의
    최종 결과(평균·표준편차)를 다른 실험과 한눈에 비교할 수 있도록 남기기 위함이다.
    """
