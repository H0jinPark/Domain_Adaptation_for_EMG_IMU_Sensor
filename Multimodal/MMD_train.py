"""MMD 학습 스크립트 (멀티모달 intermediate fusion, multi-seed 지원).

EMG/IMU 를 분리 입력받는 intermediate fusion 백본의 512차원 융합 feature 에
MMD 손실(gaussian-kernel maximum mean discrepancy)을 추가해, Source(Samsung1)
라벨만으로 Target(Samsung2) 운동 분류 성능을 끌어올린다.
...
def train_mmd(seed=42, epochs=30, batch_size=64, lr=1e-3, lambda_mmd=0.5, save_cm=False):
    ...
    save_path = os.path.join(WEIGHT_DIR, f"mm_mmd_seed{seed}_best_model.pth")
    ...
    if save_cm:
        ...
        save_confusion_matrix(
            ...
            os.path.join(RESULT_DIR, f"mm_mmd_seed{seed}_source_cm.png"),
            f"MMD MM Source (seed={seed}, Val Acc: {best_val_acc:.1f}%)", cmap="Blues")
        save_confusion_matrix(
            t_true, t_preds, class_names,
            os.path.join(RESULT_DIR, f"mm_mmd_seed{seed}_target_cm.png"),
            f"MMD MM Target (seed={seed}, Src: {best_val_acc:.1f}% vs Tgt: {best_target_acc:.1f}%)",
            cmap="Blues")
...
if __name__ == "__main__":
    ...
    if args.multi_seed:
        ...
        summarize_results(results, method_name="MMD (Multimodal)",
                          save_path=os.path.join(RESULT_DIR, "mm_mmd_summary.txt"))
    else:
        train_mmd(
