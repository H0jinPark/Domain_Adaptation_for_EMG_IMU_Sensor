...
from CDAN.CDAN_MM_model import CDAN_MM
...
def evaluate(model, loader, device):
    ...
    for emg, imu, y in loader:
        ...
        cls_out, _, _ = model(emg, imu, alpha=0.0)
...
def train(seed=42, epochs=30, batch_size=64, lr=1e-3):
    ...
    print(f"\n{'='*55}")
    print(f" CDAN-MM Training  |  seed={seed}  device={device}")
    print(f"{'='*55}")
    ...
    weight_path = f'weights/cdan_mm_seed{seed}_best.pth'

    model         = CDAN_MM(num_classes=num_classes).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for _ in range(n_iter):
            ...
            src_cls, src_dom_emg, src_dom_imu = model(src_emg, src_imu, alpha)
            _,       tgt_dom_emg, tgt_dom_imu = model(tgt_emg, tgt_imu, alpha)
    ...
    save_confusion_matrix(src_labels, src_preds, class_names,
                          f'results/cdan_mm_seed{seed}_source_cm.png',
                          f'CDAN-MM Source (seed={seed}, acc={src_acc:.4f})')
    save_confusion_matrix(tgt_labels, tgt_preds, class_names,
                          f'results/cdan_mm_seed{seed}_target_cm.png',
                          f'CDAN-MM Target (seed={seed}, acc={tgt_acc:.4f})')
