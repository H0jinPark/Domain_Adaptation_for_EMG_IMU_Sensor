"""MMD 학습 스크립트 (단일 백본, 5채널 동기화 입력, multi-seed 지원).

TCN baseline 백본의 feature 에 MMD 손실(gaussian-kernel maximum mean
discrepancy)을 추가해 Source(Samsung1) 라벨만으로 Target(Samsung2) 운동 분류
성능을 끌어올린다. 각 배치는 Source / Target 을 분리 forward 한다.
...
def gaussian_kernel(source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    """Source/Target 샘플에 여러 bandwidth 의 가우시안 커널을 합산해 반환한다."""
...
def mmd_loss(source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    """Source/Target feature 분포 간 maximum mean discrepancy 를 계산한다."""
...
# ----------------------------------------------------------------------
# 백본 래퍼: (분류 logits, feature) 를 함께 반환
# ----------------------------------------------------------------------
class MMDWrapper(nn.Module):
    """TCN baseline 백본을 감싸 (분류 logits, feature) 를 함께 반환한다."""
    ...
    def forward(self, x):
        ...
        features = self.backbone.layers(x).squeeze(-1)
...
# 단일 seed 학습
# ----------------------------------------------------------------------
def train_mmd(seed=42, epochs=30, batch_size=64, lr=1e-3, lambda_mmd=0.5, save_cm=False):
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    os.makedirs(WEIGHT_DIR, exist_ok=True)
    os.makedirs(RESULT_DIR, exist_ok=True)
    save_path = os.path.join(WEIGHT_DIR, f"5ch_mmd_seed{seed}_best_model.pth")
    ...
    print("\n" + "=" * 60)
    print(f"MMD Alignment Training Start  |  seed={seed}")
    print("Mode: 5-channel input, source/target separate forward")
    ...
    if save_cm:
        ...
        save_confusion_matrix(
            ...
            os.path.join(RESULT_DIR, f"5ch_mmd_seed{seed}_source_cm.png"),
            f"MMD 5ch Source (seed={seed}, Val Acc: {best_val_acc:.1f}%)", cmap="Blues")
        save_confusion_matrix(
            t_true, t_preds, class_names,
            os.path.join(RESULT_DIR, f"5ch_mmd_seed{seed}_target_cm.png"),
            f"MMD 5ch Target (seed={seed}, Src: {best_val_acc:.1f}% vs Tgt: {best_target_acc:.1f}%)",
            cmap="Blues")
    ...
    if args.multi_seed:
        ...
        for seed in args.seeds:
            results.append(train_mmd(
                ...
                lr=args.lr, lambda_mmd=args.lambda_mmd, save_cm=not args.no_cm))
        summarize_results(results, method_name="MMD (5-channel)",
                          save_path=os.path.join(RESULT_DIR, "5ch_mmd_summary.txt"))
    else:
        train_mmd(
