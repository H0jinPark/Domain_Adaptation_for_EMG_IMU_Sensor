"""멀티모달(EMG/IMU 분리) 전처리 — samsung2_original (raw, unaligned) 버전.

`data_preprocess_MM.py` 와 모든 로직이 동일하나, target 으로
`samsung2_original.parquet` (YXZ 재라벨링 *안 된* 원본) 을 사용한다.
출력은 `preprocessed_MM_original/` 로 저장.

앞으로의 멀티모달 실험(InterFusion / DANN-MM / CDAN-MM 등)은
source=samsung1, target=samsung2_original 조합을 이 디렉토리에서 로드한다.

채널 규약
  EMG: [biceps, triceps]                 -> 1000Hz, window=5000 (5초)
  IMU: [triceps_X, triceps_Y, triceps_Z] ->  100Hz, window=500  (5초)

저장 파일 (preprocessed_MM_original/)
  X_emg_{prefix}.npy : (N, 2, 5000)
  X_imu_{prefix}.npy : (N, 3,  500)
  y_{prefix}.npy     : (N,)

차이 요약 (vs `data_preprocess_MM.py`):
  - target parquet:  `data/samsung2.parquet` → `data/samsung2_original.parquet`
  - 출력 디렉토리:    `preprocessed_MM/`      → `preprocessed_MM_original/`
나머지 (resample 1000Hz, EMG 20-450Hz bandpass, 모달별 z-score,
EMG 5000 / IMU 500 윈도우, 세션 8:2 split seed42) 모두 동일.
"""
import numpy as np
import pandas as pd
...
OUT_DIR    = 'preprocessed_MM_original'
...
if __name__ == "__main__":
    ...
    print("\n[Step 2] Target Data (samsung2_original)...")
