"""DANN 학습 스크립트 (단일 백본, 5채널 동기화 입력, discriminator BN 제거).
...
이 버전은 기존 DANN 과 동일하게 source/target 을 분리 forward 하지만,
domain discriminator 내부 BatchNorm 을 제거한 모델을 사용한다.
따라서 AdaTime-style DANN 과 더 가까운 비교군으로 사용할 수 있다.
"""
import os
import sys
import random
...
from DANN.DANN_experiment.DANN_model_no_disc_bn import DANNModel
...
def train_dann(seed=42, epochs=30, batch_size=64, lr=1e-3, domain_weight=1.0, save_cm=True):
    ...
    save_path = f'weights/dann_no_disc_bn_seed{seed}_best_model.pth'
    ...
    print("\n" + "=" * 60)
    print(f"DANN Training Start  |  seed={seed}")
    print("Mode: separate-forward, no BatchNorm in discriminator")
        ...
        for i, ((src_x, src_y), (tgt_x, tgt_y)) in pbar:
            ...
            with torch.no_grad():
                domain_pred = torch.cat([
                    src_domain_out.argmax(dim=1),
                    tgt_domain_out.argmax(dim=1)
                ], dim=0)
    ...
    if save_cm:
        ...
        plt.title(f'DANN No Disc BN Source Prediction\n(seed={seed}, Val Acc: {best_val_acc:.1f}%)', fontsize=16)
        plt.xlabel('Predicted Label', fontsize=12)
        plt.ylabel('True Label', fontsize=12)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(f'results/dann_no_disc_bn_seed{seed}_source_confusion_matrix.png', dpi=300)
        plt.close()
        ...
        plt.title(f'DANN No Disc BN Target Prediction\n(seed={seed}, Source Val: {best_val_acc:.1f}% vs Target: {best_target_acc:.1f}%)', fontsize=16)
        plt.xlabel('Predicted Label', fontsize=12)
        plt.ylabel('True Label', fontsize=12)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(f'results/dann_no_disc_bn_seed{seed}_target_confusion_matrix.png', dpi=300)
        plt.close()
...
def summarize_results(results):
    ...
    print(
        f"{'Std':>6} | "
        f"{source.std(ddof=ddof):>8.2f} | "
        f"{target.std(ddof=ddof):>8.2f} | "
        f"{shift.std(ddof=ddof):>8.2f} | "
        f"{domain_loss.std(ddof=ddof):>8.4f} | "
        f"{domain_acc.std(ddof=ddof):>7.2f}%"
    )
    print("=" * 72)
