...
class tf_decoder(nn.Module):
    def __init__(self, configs):
        ...
        # 단계적 업샘플링: [B,128,1] → [B,5,5000]
        # ConvTranspose1d(128, 5000, k=5) 대비 파라미터 ~100배 절감 (3.2M → 31K)
        self.decoder_net = nn.Sequential(
            nn.Upsample(size=50),
            nn.Conv1d(configs.final_out_channels, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Upsample(size=500),
            nn.Conv1d(64, 32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Upsample(size=self.sequence_len),
            nn.Conv1d(32, self.input_channels, kernel_size=3, padding=1),
        )
    ...
    def forward(self, f, out_ft):
        ...
        et = f[:, self.modes * 2:]
        x_high = self.decoder_net(et.unsqueeze(2))
        x_high = F.relu(self.bn2(x_high))
