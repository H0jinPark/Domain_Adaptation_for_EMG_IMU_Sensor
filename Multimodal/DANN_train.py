"""DANN 학습 스크립트 (멀티모달 intermediate fusion, multi-seed 지원).

EMG/IMU 를 분리 입력받는 intermediate fusion 백본 위에 Gradient Reversal Layer
기반 domain discriminator 를 얹어, Source(Samsung1) 라벨만으로 Target(Samsung2)
운동 분류 성능을 끌어올린다.

규약
  - 각 배치는 Source / Target 을 분리 forward 한다 (concat-forward 아님).
  - domain discriminator 내부에는 BatchNorm 을 두지 않는다 (mm_model.py 참고).
...
from mm_model import InterFusionDANN
...
def train_dann(seed=42, epochs=30, batch_size=64, lr=1e-3, domain_weight=1.0, save_cm=True):
    ...
    save_path = os.path.join(WEIGHT_DIR, f"mm_dann_seed{seed}_best_model.pth")

    train_loader, val_loader, tgt_train_loader, tgt_val_loader, num_classes, le = \
        get_mm_dataloaders(batch_size=batch_size)
    class_names = le.classes_

    model = InterFusionDANN(num_classes=num_classes).to(device)
    ...
    if save_cm:
        ...
        save_confusion_matrix(
            ...
            os.path.join(RESULT_DIR, f"mm_dann_seed{seed}_source_cm.png"),
            f"DANN MM Source (seed={seed}, Val Acc: {best_val_acc:.1f}%)", cmap="Blues")
        save_confusion_matrix(
            t_true, t_preds, class_names,
            os.path.join(RESULT_DIR, f"mm_dann_seed{seed}_target_cm.png"),
            f"DANN MM Target (seed={seed}, Src: {best_val_acc:.1f}% vs Tgt: {best_target_acc:.1f}%)",
            cmap="Blues")
...
if __name__ == "__main__":
    ...
    if args.multi_seed:
        ...
        summarize_results(results, method_name="DANN (Multimodal)",
                          save_path=os.path.join(RESULT_DIR, "mm_dann_summary.txt"))
    else:
        train_dann(
