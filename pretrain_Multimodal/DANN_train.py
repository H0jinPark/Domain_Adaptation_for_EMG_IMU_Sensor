"""Phase 2: DANN 학습 (pretrained encoder 로드 후 fine-tune + discriminator).

ssl_train.py (Phase 1)로 저장된 EMG/IMU 인코더 가중치를 로드하고,
intermediate fusion backbone + GRL 기반 domain discriminator를 end-to-end fine-tune.

규약
  - --pretrain_seed: Phase 1에서 사용한 seed (기본 0)
  - Phase 1 가중치가 없으면 오류 없이 무작위 초기화로 진행하되 경고를 출력한다.
  - source/target 분리 forward, discriminator 내부 BatchNorm 없음.
  - Source val 기준으로 best 모델 저장.
"""
import os
import sys
import argparse
...
from pretrain_Multimodal.pretrain_model import PretrainInterFusionDANN
...
def train_dann(seed=42, pretrain_seed=0, epochs=30, batch_size=64,
               lr=1e-3, domain_weight=1.0, save_cm=True):
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    os.makedirs(WEIGHT_DIR, exist_ok=True)
    os.makedirs(RESULT_DIR, exist_ok=True)

    save_path = os.path.join(WEIGHT_DIR, f"pt_dann_seed{seed}_best_model.pth")

    train_loader, val_loader, tgt_train_loader, tgt_val_loader, num_classes, le = \
        get_pm_dataloaders(batch_size=batch_size)
    class_names = le.classes_

    model = PretrainInterFusionDANN(num_classes=num_classes)
    ...
    if os.path.exists(emg_path) and os.path.exists(imu_path):
        ...
        print(f"    expected: {emg_path}")
        print(f"    expected: {imu_path}")
    ...
    print("\n" + "=" * 60)
    print(f"Phase 2 DANN Training Start  |  seed={seed}  pretrain_seed={pretrain_seed}")
    print("Mode: pretrained encoders + intermediate fusion + GRL discriminator")
    ...
    if save_cm:
        ...
        save_confusion_matrix(
            ...
            os.path.join(RESULT_DIR, f"pt_dann_seed{seed}_source_cm.png"),
            f"PT-DANN Source (seed={seed}, Val: {best_val_acc:.1f}%)", cmap="Blues")
        save_confusion_matrix(
            t_true, t_preds, class_names,
            os.path.join(RESULT_DIR, f"pt_dann_seed{seed}_target_cm.png"),
            f"PT-DANN Target (seed={seed}, Src: {best_val_acc:.1f}% / Tgt: {best_target_acc:.1f}%)",
            cmap="Blues")
        print("혼동 행렬 저장 완료.")
...
def parse_args():
    ...
    parser.add_argument("--pretrain_seed", type=int, default=0,
                        help="Phase 1 SSL 학습에 사용한 seed (가중치 파일 선택용)")
...
if __name__ == "__main__":
    ...
    if args.multi_seed:
        ...
        summarize_results(results, method_name="DANN (Pretrain-MM)",
                          save_path=os.path.join(RESULT_DIR, "pt_dann_summary.txt"))
    else:
        train_dann(
