...
def select_result_subdir(name):
    ...
    분할 규약이 다른 실험(피험자 단위 등)의 산출물이 기존 IMU 단독 결과와
...
def select_backbone(name):
    ...
    from Compact.compact_model import IMUOnlyCDAN as CompactIMUCDAN
    BACKBONE = "compact"
    MODEL_CLS = CompactIMUCDAN
    RESULT_DIR = os.path.join(PROJECT_ROOT, "Compact", "Result", "IMU")
...
def train_cdan(seed=42, epochs=30, batch_size=64, lr=1e-3, domain_weight=1.0, save_cm=False, tag=""):
    ...
    if save_cm:
        ...
        save_confusion_matrix(
            ...
            os.path.join(RESULT_DIR, f"imu_cdan{suffix}_seed{seed}_source_test_cm.png"),
            f"CDAN IMU Source TEST (seed={seed}, Acc: {src_test_acc:.1f}%)", cmap="Greens")
        save_confusion_matrix(
            t_true, t_preds, class_names,
            os.path.join(RESULT_DIR, f"imu_cdan{suffix}_seed{seed}_target_test_cm.png"),
            f"CDAN IMU Target TEST (seed={seed}, Src: {src_test_acc:.1f}% vs Tgt: {tgt_test_acc:.1f}%)",
            cmap="Greens")
