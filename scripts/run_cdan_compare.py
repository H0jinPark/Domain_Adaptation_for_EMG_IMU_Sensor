#!/usr/bin/env python
"""축 정렬 메서드별 CDAN 학습 + 결과 비교 드라이버.

각 메서드(raw/pca/gravity/permutation/kabsch)를 **별도 프로세스**로 학습한다.
메서드 전환은 환경변수 MM_DATA_DIR 로 하는데, 이 값은 mm_data_loader 가 import
시점에 한 번만 읽으므로 한 프로세스 안에서 메서드를 바꿀 수 없다 → subprocess 분리.

학습이 끝나면 각 메서드가 남긴 results/cdan_result_<method>.json 을 모아
  - 콘솔 표
  - results/cdan_method_comparison.csv
  - results/cdan_method_comparison.png  (target accuracy 막대 + 표준편차 에러바)
로 비교한다.

사용 예
  # 5개 메서드 모두, 5 seed 평균
  python scripts/run_cdan_compare.py --multi_seed --epochs 30
  # 빠른 점검: 단일 seed, 일부 메서드만
  python scripts/run_cdan_compare.py --methods raw gravity --seed 42 --epochs 30 --no_cm
  # 이미 학습 끝났고 비교만 다시
  python scripts/run_cdan_compare.py --compare_only
"""
import argparse
import json
import os
...
CDAN_TRAIN = os.path.join(ROOT, "Multimodal", "CDAN_train.py")
...
def run_one(method, args):
    ...
    print("\n" + "#" * 72)
    print(f"# CDAN  method={method}   MM_DATA_DIR={data_dir}")
    print("#" * 72, flush=True)
    return subprocess.run(cmd, env=env, cwd=ROOT).returncode == 0


def load_results(methods):
    """메서드별 cdan_result_<method>.json 로드 → 행 리스트(타깃 정확도 내림차순)."""
    rows = []
    for m in methods:
        p = os.path.join(RESULT_DIR, f"cdan_result_{m}.json")
...
def compare(methods):
    ...
    csv_path = os.path.join(RESULT_DIR, "cdan_method_comparison.csv")
    ...
    colors = ["tab:gray" if m == "raw" else "tab:blue" for m in names]
    ...
    ax.set_title("CDAN target accuracy by IMU axis-alignment method")
    plt.tight_layout()
    png_path = os.path.join(RESULT_DIR, "cdan_method_comparison.png")
    plt.savefig(png_path, dpi=200)
    print(f"그래프 저장: {png_path}")


def parse_args():
    p = argparse.ArgumentParser(description="메서드별 CDAN 학습 + 비교")
