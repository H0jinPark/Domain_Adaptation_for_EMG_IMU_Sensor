"""CDAN 학습 스크립트 (단일 백본, 5채널 동기화 입력, multi-seed 지원).

Conditional Domain Adversarial Network 로 Source(Samsung1) 라벨만 사용해
...
def train_cdan(seed=42, epochs=30, batch_size=64, lr=1e-3, domain_weight=1.0, save_cm=True):
    ...
    save_path = os.path.join(WEIGHT_DIR, f"5ch_cdan_seed{seed}_best_model.pth")
    ...
    if save_cm:
        ...
        save_confusion_matrix(
            ...
            os.path.join(RESULT_DIR, f"5ch_cdan_seed{seed}_source_cm.png"),
            f"CDAN 5ch Source (seed={seed}, Val Acc: {best_val_acc:.1f}%)", cmap="Purples")
        save_confusion_matrix(
            t_true, t_preds, class_names,
            os.path.join(RESULT_DIR, f"5ch_cdan_seed{seed}_target_cm.png"),
            f"CDAN 5ch Target (seed={seed}, Src: {best_val_acc:.1f}% vs Tgt: {best_target_acc:.1f}%)",
            cmap="Purples")
...
if __name__ == "__main__":
    ...
    if args.multi_seed:
        ...
        summarize_results(results, method_name="CDAN (5-channel)",
                          save_path=os.path.join(RESULT_DIR, "5ch_cdan_summary.txt"))
    else:
        train_cdan(
